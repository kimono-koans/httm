...
impl RollForward {
  pub fn new(roll_config: RollForwardConfig) -> HttmResult<Self> {
    let (dataset_name, snap_name) = if let Some(res) = roll_config.full_snap_name.split_once('@') {
      res
    } else {
    ...
    match diff_type {
      ...
      Some(&"R") => {
        let new_file_name = split_line
          .get(3)
          .ok_or_else(|| HttmError::new("Could not obtain a new file name for diff event."))?;
  ...
  fn diff_action(&self, event: &DiffEvent) -> HttmResult<()> {
    ...
    match &event.diff_type {
      ...
      DiffType::Renamed(new_file_name) => {
        let snap_new_file_name = self
          .snap_path(new_file_name)
          .ok_or_else(|| HttmError::new("Could not obtain snap file path for live version."))?;
...
impl<'a> PreserveHardLinks<'a> {
      ...
      .try_for_each(|(_key, values)| {
        ...
        complemented_paths
          ...
          .try_for_each(|(live_path, snap_path)| {
            ...
            match opt_original {
              Some(original) if original == live_path => RollForward::copy(snap_path, live_path),
  ...
  fn diff_orphans(&'a self) -> HttmResult<HashSet<PathBuf>> {
    ...
    snap_diff.clone().par_bridge().try_for_each(|live_path| {
      let snap_path: HttmResult<PathBuf> = RollForward::snap_path(self.roll_forward, live_path)
        .ok_or_else(|| HttmError::new("Could obtain live path for snap path").into());
