    ...
    let browse_result = match &GLOBAL_CONFIG.opt_requested_dir {
      ...
      Some(requested_dir) => {
        ...
        if browse_result.selected_pathdata.is_empty() {
          return Err(
            HttmError::new("None of the selected strings could be converted to paths.").into(),
          );
        }
...
impl InteractiveSelect {
  fn exec(browse_result: InteractiveBrowse, interactive_mode: &InteractiveMode) -> HttmResult<()> {
      ...
      loop {
        ...
        if let Some(path_string) = broken_string.get(1) {
          ...
          if display_map
            .map
            .iter()
            .all(|(live_version, _snaps)| Path::new(path_string) != live_version.path_buf.as_path())
          {
    ...
    let last_snap = versions_map
      ...
      .filter(|snap_version| {
        if GLOBAL_CONFIG.opt_omit_ditto {
          snap_version.md_infallible().modify_time != live_version.md_infallible().modify_time
        } else {
          true
        }
      })
...
impl ViewMode {
  ...
  fn browse(&self, requested_dir: &Path) -> HttmResult<InteractiveBrowse> {
    ...
    let display_handle = thread::spawn(move || {
      let opt_multi = GLOBAL_CONFIG.opt_last_snap.is_none() && GLOBAL_CONFIG.opt_preview.is_none();
