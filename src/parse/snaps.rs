...
use crate::{
    BTRFS_SNAPPER_HIDDEN_DIRECTORY, BTRFS_SNAPPER_SUFFIX, MAIN_POOL, ZFS_SNAPSHOT_DIRECTORY,
};
...
impl MapOfSnaps {
    ...
    pub fn new(map_of_datasets: &HashMap<PathBuf, DatasetMetadata>) -> HttmResult<Self> {
        ...
        let map_of_snaps: HashMap<PathBuf, Vec<PathBuf>> = MAIN_POOL.install(|| {
            map_of_datasets
                .par_iter()
                        ...
                        FilesystemType::Btrfs => {
                                ...
                                .expect("get_or_init() should have set this value")
                            {
                                Some(root_mount_path) => match dataset_info.mount_type {
                                    MountType::Local => {
                                        Self::from_btrfs_cmd(mount, root_mount_path)
                                    }
                                    MountType::Network => {
                                        Self::from_defined_mounts(mount, dataset_info)
                                    }
    ...
    fn from_defined_mounts(
        ...
        MAIN_POOL.install(|| match dataset_metadata.fs_type {
            FilesystemType::Btrfs => Ok(read_dir(
            ...
            .collect()),
            FilesystemType::Zfs => Ok(read_dir(mount_point_path.join(ZFS_SNAPSHOT_DIRECTORY))?
                .flatten()
                .par_bridge()
