...
impl MapOfSnaps {
  ...
  fn from_btrfs_cmd(mount: &Path) -> HttmResult<Vec<PathBuf>> {
    ...
    let command_output =
      std::str::from_utf8(&ExecProcess::new(exec_command).args(&args).output()?.stdout)?.to_owned();
        ...
        snap_paths
          ...
          .map(|relative| {
            if pre.contains("<FS_TREE>") {
              ...
              let snap_path_parsed: PathBuf = Path::new(relative).components().skip(1).collect();
