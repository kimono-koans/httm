...
use crate::{interactive::SelectionCandidate, FilesystemType, SnapPoint};
use crate::{BasicDirEntryInfo, Config, HttmError, PathData};
...
pub fn take_snapshot(
    ...
    fn exec_snapshot(
        config: &Config,
        zfs_command: &PathBuf,
        mounts_for_files: &Vec<PathData>,
    ) -> Result<[Vec<PathData>; 2], Box<dyn std::error::Error + Send + Sync + 'static>> {
        let exec_command = zfs_command;
        let now = SystemTime::now();

        let res: Result<(), HttmError> = mounts_for_files
            .iter()
            .try_for_each(|mount| {
